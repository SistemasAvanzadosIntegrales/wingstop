...
Snog.define("models", function (require, exports, module) {
    ...
    exports.BatchModel = function () {
        ...
        this.setBatch = function (response_message) {
            ...
            this.max_picks = response_message.max_picks !== null ? response_message.max_picks : null;
...
Snog.define("data", function (require, exports, module) {
            ...
            for (prop in this) {
                if(this.hasOwnProperty(prop)) {
                    ...
                    if(type === "object") {
                        ...
                        if(type === "boolean" && prop !== "keepUserLoggedIn") {
                            this[prop] = false;
                        }
                    }
                }
    ...
    if (sessionStorage && sessionStorage.getItem('snog_auth_token')){
        module.exports.auth_token = sessionStorage.getItem('snog_auth_token');
    }
});/**
    ...
    module.exports = {
        ...
        on_login_success: function (response, request) {
            ...
            if (jQuery.cookie && snog_data.keepUserLoggedIn) {
                ...
                window.sessionStorage.setItem('snog_auth_token', snog_data.auth_token);
            }
        ...
        logout_success_callback: function (response, request) {
            ...
            sessionStorage.removeItem('snog_auth_token');
                // clear cookies data;
            if ( jQuery.cookie ) {
                if (sessionStorage && sessionStorage.getItem('snog_auth_token') === jQuery.cookie('snog_auth_token')){
                    try {
...
Snog.define("handlers", function (require, exports, module) {
    ...
    snog_dispatcher.on(Snog.events.LOGIN, function (data) {
        if (snog_api.isConnected() && data ) {
            ...
            var eoms = (data.error_on_multiple_sessions === "" || data.error_on_multiple_sessions === undefined) ? null: data.error_on_multiple_sessions;
            var eos = (data.end_other_sessions === "" || data.end_other_sessions === undefined) ? null: data.end_other_sessions;
            if (e !== null && p !== null) {
                ...
                // send request to the server;
                var request = snog.loginMessage(e, p, eoms, eos);
                snog_api._authorizing = true;
     ...
     *    data params: null;
     */
    snog_dispatcher.on(Snog.events.AUTO_LOGIN, function (data) {
        var eoms = (data.error_on_multiple_sessions === "" || data.error_on_multiple_sessions === undefined) ? null: data.error_on_multiple_sessions;
        var eos = (data.end_other_sessions === "" || data.end_other_sessions === undefined) ? null: data.end_other_sessions;
        ...
        if (sessionStorage){
            if (sessionStorage.snog_auth_token){
                snog_data.auth_token = sessionStorage.snog_auth_token;
            }
        }

        if ((snog_data.player_id !== null || snog_data.player_email !== null) && snog_data.auth_token !== null) {
            
            var request = "";
            if (snog_data.player_id !== null){
                request = snog.autoLoginMessage(snog_data.player_id, snog_data.auth_token, eoms, eos);
            }else{
                request = snog.autoLoginWithEmailMessage(snog_data.player_email, snog_data.auth_token, eoms, eos);
            }
    ...
    snog_dispatcher.on(Snog.events.GET_ITEM_BATCH, function (data) {
        if (data && data.hasOwnProperty("size") && data.hasOwnProperty("ttl")) {
            ...
            if (data && data.hasOwnProperty("batch_uuid")){
                var request = snog.getItemBatchWithUUIDMessage(data.batch_uuid);
                snog_api.applyAJAX(request, 'item_instance_batch', ['invalid_batch_request', 'item_batch'], null);
            }else{
...
Snog.define("realm", function (require, exports, module) {
    ...
    snog_dispatcher.on(Snog.events.REALM_LOGIN, function (data) {
        ...
        var eoms = (data.error_on_multiple_sessions ? data.error_on_multiple_sessions : null);
        var eos = (data.end_other_sessions ? data.end_other_sessions : null);

        if (data.token !== null) {
            var request = snog.realmLoginMessage(exports.realm, data.token, eoms, eos);
            snog_api._authorizing = true;
 ...
 */
snog.autoLoginMessage = function (playerID, authToken, error_on_multiple_sessions, end_other_sessions) {
    var msg = { 
        message_type : "login",
        player_id : playerID,
        auth_token : authToken,
        error_on_multiple_sessions : error_on_multiple_sessions,
        end_other_sessions : end_other_sessions
    };
    return msg;
}
 ...
 * auth_token : The user's authentication token.
 */
snog.autoLoginWithEmailMessage = function (email, authToken, error_on_multiple_sessions, end_other_sessions) {
    var msg = { 
        message_type : "login",
        email : email,
        auth_token : authToken,
...
/** Retrieves a batch of item instances, asking the server to reserve them for ttl seconds.
 *
 * batch_uuid: the uuid of a server batch
 */
snog.getItemBatchWithUUIDMessage = function (batch_uuid){
    var msg = {
        message_type : "get_item_batch",
 ...
 */
snog.loginMessage = function (email, password, error_on_multiple_sessions, end_other_sessions) {
    var msg = { 
        message_type : "login",
        email : email,
        password : password,
...
/** Log a user in the system with realm credentials (realm and access_token).
 *
 * realm: The access realm  
 * access_token: The user's access token
 */
snog.realmLoginMessage = function
 (realm, access_token, error_on_multiple_sessions, end_other_sessions) {
    var msg = { 
        message_type : "login",
        realm: realm,
 ...
 *
 */
snog.redeemCodeMessage = function (key) {
    var msg = { 
